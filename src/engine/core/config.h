...
namespace engine::core
{
    ...
    public:
        ...
        explicit Config(std::string filepath); // 构造函数，指定配置文件路径。
        ...
        bool loadFromFile(std::string filepath);             // 从指定的 JSON 文件加载配置。成功返回 true，否则返回 false。
        [[nodiscard]] bool saveToFile(std::string filepath); // 当前配置保存到指定的 JSON 文件。成功返回 true，否则返回 false。
